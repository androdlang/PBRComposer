...
LiteGraph.hexToColor = function( color_hex, array)
{
    ...
    if(!array)
        return "new BABYLON.Color3("+(color[0]/255).toFixed(3)+","+(color[1]/255).toFixed(3)+","+(color[2]/255).toFixed(3)+")";
...
LGraph.prototype.selectTexture = function(node)
{
    if (this._nodes_by_id[node.id] == null)
        return; //not found
    if (this.onSelectTexture)
        this.onSelectTexture(node);
...
LGraph.prototype.loadFromData = function (data, on_pre_configure, on_complete, params) {
...
LGraphNode.prototype.addBasicProperties = function(  )
{
    ...
    //this.options.global_name = {hidden:false};//!this.properties.is_global};
...
LGraphCanvas.prototype.onNodeDblClicked = function (n) {
    //console.log("DOUBLECLICK:" + n.id);
    var element = document.getElementById("Details");
    var eclass = element.getElementsByClassName("cr function");
    eclass[0].click();
}
        ...
        if (node.inputs)
            for (var i = 0; i < node.inputs.length; i++) {
                ...
                if (render_text) {
                    ...
                    if (text) {
                        var colorNA = LiteGraph.NODE_COLOR_NOTALLOWED;
                        if (node.title=='Output') {
                            var mm = node.properties.MetallicModel;
                            if (i == 9) { //reflectivityTexture
                                ctx.fillStyle = mm?colorNA:color;
                            } else if (i == 10) { //metallicTexture
                                ctx.fillStyle = mm ? color : colorNA;
                            } else if (i == 1) { //reflectivityColor
                                ctx.fillStyle = mm ? colorNA : color;
                            } else if (i == 3) { //ambientColor
                                ctx.fillStyle = mm ? colorNA : color;
                            }
...
LGraphCanvas.onMenuSelectTexture = function (node) {
   // if (node.removable == false) return;
    node.graph.selectTexture(node);
    console.log("onMenuSelectTexture");
    node.setDirtyCanvas(true, true);
}
...
LGraphCanvas.prototype.getNodeMenuOptions = function (node) {
    ...
    //options.push(null, { content: "SetTexture", callback: LGraphCanvas.onMenuSelectTexture });
...
//    for (var line in offset.fragment.includes) { includes[line] = 1; }
    ...
    r +="      //ambient_color = "+albedo.getOutputVar()+";\n"
    //ambient light
...
//        "      vec3 specular_reflection =   mix( diffuse_reflection, vec3(1.0), w) ;\n"; // vec3(1.0 is the light color)
    ...
    r +="      gl_FragColor = vec4( emission + refraction_color +"+  " specular_color + (ambient_light + diffuse_light) * diffuse_color, "+ alpha_value +" );\n" +
        "}";
...
PUVs.getFragmentCode = function (out_var, utiling, vtiling, uoffset, voffset, scope) {
    if (scope == CodePiece.FRAGMENT || scope == CodePiece.BOTH) {
        console.log(".uOffset = " + utiling +";\n.vOffset = " + vtiling +";\n");
        //return "vec2 " + out_var + " = v_coord * vec2(" + utiling + "," + vtiling + ");\n";
        return ("var uvcoords= { uScale:" + utiling + ",vScale:" + vtiling + ",uOffset:" + uoffset + ",vOffset:" + voffset+'}\n');
    }
    return "";
}
...
PUVs.getCode = function (params) {
    ...
    var uoffset = params.uoffset || "0.000";
    var voffset = params.voffset || "0.000";
    var scope = params.scope;
    var order = params.hasOwnProperty("order") ? params.order : Number.MAX_VALUE;

    var uvs_modified = (utiling !== "1.000" || vtiling !== "1.000" || uoffset !== "0.000" || voffset !== "0.000");

    var fragment = new CodePiece(order);
    if(uvs_modified)
        fragment.setBody(this.getFragmentCode(out_var, utiling, vtiling, uoffset, voffset, scope));
...
LiteGraph.CodeLib["bjsdummy"] = new POperation(undefined, "+");
...
PTextureSampleCube.getFragmentCode = function (output, input, texture_id) {
    ...
    code += "//BJS SampleCube:" + texture_id+'\n';
    return code;
}
...
PTextureSample.getVertexCode = function (output, input, texture_id, texture_type, scope, order) {
    ...
    if(scope == CodePiece.VERTEX) {
        ...
        code_str += "//BABYLON.Texture " + output + " = texture2D(" + texture_id + ", " + input + ");\n";
...
PTextureSample.getFragmentCode = function (output, input, texture_id, texture_url, texture_type, scope, order) {
    input = input || "v_coord";
    var code = new CodePiece(order);
    ...
    if(scope == CodePiece.FRAGMENT) {
        ...
        code.addHeaderLine("var " + texture_id + "= new BABYLON.Texture('"+ texture_url +  "',bscene);\n");
        code.addHeaderLine("var " + output + " = " + texture_id + ";\n");


        //if( texture_type == LiteGraph.COLOR_MAP || texture_type == LiteGraph.SPECULAR_MAP) {
        //code_str = "vec4 " + output + " = texture2D(" + texture_id + ", " + input + ");\n";

        code_str = 'var ' + output + " = " + texture_id + ";\n";

        /*
        bjs_str="//var "+output+" = "+ texture_id + ";\n";
...
PTextureSample.getCode = function (params) {
    ...
    var input = params.input;
    var texture_id = params.texture_id;
    var texture_url = params.texture_url;
    ...
    var fragment = this.getFragmentCode(out_var, input, texture_id, texture_url,texture_type, scope, order);
